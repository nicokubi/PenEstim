...
mhChain_v7.1 <- function(seed, n_iter, burn_in, chain_id, data, max_age, db,
  ...
  for (i in 1:n_iter) {
    ...
    # Apply symmetric reflection for the asymptote values
    proposal_vector[1] <- ifelse(proposal_vector[1] < 0, -proposal_vector[1], ifelse(proposal_vector[1] > 1, 2 - proposal_vector[1], proposal_vector[1]))
    proposal_vector[2] <- ifelse(proposal_vector[2] < 0, -proposal_vector[2], ifelse(proposal_vector[2] > 1, 2 - proposal_vector[2], proposal_vector[2]))
...
PenEstim_v7.1 <- function(data, cancer_type, gene_input, n_chains = 1,
  ...
  parallel::clusterExport(cl, c(
    "mhChain_v7.1", "mhLogLikelihood_clipp", "calculate_lifetime_risk", "calculateNCPen",
