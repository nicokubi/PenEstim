...
#' @export
#'
calculateNCPen <- function(SEER_baseline, alpha, beta, delta, af, max_age, homozygote) {
    ...
    for (age in 1:max_age) {
        ...
        carrierRisk <- dweibull(age - delta, shape = alpha, scale = beta) 
...
lik.fn <- function(i, data, alpha_male, alpha_female, beta_male, beta_female, 
                   delta_male, delta_female, max_age,
  ...
  # Map sex to row index: "Female" is 1st row and "Male" is 2nd row
    ...
    if (data$age[i] == 0) {
        ...
        # Weibull parameters for penetrance
        survival_prob <- 1 - pweibull(max(age_index - delta, 1), shape = alpha, scale = beta) 
        c.pen <- (pweibull(max(age_index - delta, 1), shape = alpha, scale = beta)
        - pweibull(max(age_index - 1 - delta, 1), shape = alpha, scale = beta))
        ...
        if (SeerNC == TRUE) {
            ...
            nc.pen <- calculateNCPen(
                ...
                beta = beta, delta = delta, max_age = max_age,
...
mhLogLikelihood_clipp <- function(paras, families, max_age, cancer_type, db, af, homozygote, SeerNC, ncores) {
  ...
  delta_female <- paras[2]
    ...
    given_first_quartile_female <- paras[6]
    ...
    # We assume a single bi-allelic locus where the homozygous carrier is not viable
    if (homozygote) {
    geno_freq <- c(1 - af, af)
    trans <- matrix(c(1, 0,
                         0.5, 0.5,
    ...
    # Core usages if not specified 
    # Set ncores to the number of cores detected minus one if not specified
     if (is.null(ncores)) {
         ncores <- detectCores() - 1 # Reserve one core for system processes
     }

    # Compute log-likelihood
    loglik <- pedigree_loglikelihood(families, geno_freq, trans, lik, ncores = ncores) 

    # Handle -Inf values in the loglikelihood
