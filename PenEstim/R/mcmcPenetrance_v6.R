...
#' @export
#'
mhChain_v6 <- function(seed, n_iter, burn_in, chain_id, data, max_age, db,
                       prior_distributions, cancer_type, gene_input, af,
                       median_max, max_penetrance, homozygote, SeerNC, priors, ncores) {
  # Set seed
  set.seed(seed)
  ...
  var <- c(2, 2, 5, 5, 5, 5) # Example standard deviations for four parameters
  C <- diag(var)
  ...
  for (i in 1:n_iter) {
    ...
    # Generate proposals using the adaptive covariance matrix from a multivariate Normal
    proposal_vector <- mvrnorm(1, mu = params_vector, Sigma = C)

    # Update the proposal state outputs
    out$threshold_male_proposals[i] <- proposal_vector[1]
    out$threshold_female_proposals[i] <- proposal_vector[2]
    out$median_male_proposals[i] <- proposal_vector[3]
    out$median_female_proposals[i] <- proposal_vector[4]
    out$first_quartile_male_proposals[i] <- proposal_vector[5]
    out$first_quartile_female_proposals[i] <- proposal_vector[6]

    # Update the params_proposal to use the generated proposal_vector correctly:
    params_proposal <- list(
      threshold_male = max(priors$threshold$min, min(proposal_vector[1], priors$threshold$max)),
      threshold_female = max(priors$threshold$min, min(proposal_vector[2], priors$threshold$max)),
      median_male = max(proposal_vector[3], proposal_vector[1]),
      median_female = max(proposal_vector[4], proposal_vector[6]),
      first_quartile_male = max(proposal_vector[1], min(proposal_vector[3], proposal_vector[5])),
      first_quartile_female = max(proposal_vector[2], min(proposal_vector[4], proposal_vector[6]))
    )
    ...
    # Update the proposal covariance matrix after t0 iterations
    if (i > max(burn_in * n_iter, 3)) {
      # Update of the covariance matrix based on Formula 1 in Haario et al. (2001)
      C <- sd * cov(do.call(rbind, current_states)) + eps * sd * diag(num_pars)
    }

    # Update the current state output
    out$threshold_male_samples[i] <- params_vector[1]
    out$threshold_female_samples[i] <- params_vector[2]
    out$median_male_samples[i] <- params_vector[3]
    out$median_female_samples[i] <- params_vector[4]
    out$first_quartile_male_samples[i] <- params_vector[5]
    out$first_quartile_female_samples[i] <- params_vector[6]
...
PenEstim_v6 <- function(data, cancer_type, gene_input, n_chains = 4,
