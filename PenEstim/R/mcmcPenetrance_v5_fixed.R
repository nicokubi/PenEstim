...
mhChain_v5_fixed <- function(seed, n_iter, burn_in, chain_id, data, max_age, db,
...
# As a basic choice for the scaling parameter we have adopted the value sd from Gelman et al. (1996)
  ...
  for (i in 1:n_iter) {
    ...
    params_proposal <- list(
      asymptote_male = 1,
      asymptote_female = 1,
      threshold_male = max(priors$threshold$min, min(proposal_vector[3], priors$threshold$max)),
      threshold_female = max(priors$threshold$min, min(proposal_vector[4], priors$threshold$max)),
...
PenEstim_v5_fixed <- function(data, cancer_type, gene_input, n_chains = 4,
  ...
  parallel::clusterExport(cl, c(
    "mhChain_v5_fixed", "mhLogLikelihood_clipp", "calculate_lifetime_risk", "calculateNCPen",
