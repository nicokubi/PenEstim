#' Execution of Metropolis-Hastings for Cancer Risk Estimation
#'
...
#' @examples
#' result <- mhChain_v2(
#'     seed = 123, n_iter = 1000, data = familyData,
...
mhChain_v10 <- function(seed, n_iter, ncores, burn_in, data, max_age, db,
                       prior_distributions, cancer_type, gene_input, af,
                       median_max, homozygote, SeerNC, priors, var) {
    # Set seed
    set.seed(seed)
...
PenEstim_v10 <- function(data, cancer_type, gene_input,
    ...
    all_high_rejections <- results$rejection_rate
    if (all_high_rejections > 0.9) {
