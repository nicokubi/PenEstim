...
This is a guide on creating and working with pedigrees in the `PPP` R package.

The family history user input to the main `PPP` function is `pedigree`, which is a `data.frame` with certain required columns. See `help(PPP)` for more detail on the expected values for these columns. This package includes four sample pedigrees: `test_fam_1`, `test_fam_2`, `test_fam_3`, `test_fam_4` and `err_fam_1`. These examples are intended to show users different kinds of pedigrees that can be useful for illustrating the capabilities of the `PPP` package. 
...
Note that this pedigree contains so-called 'loops' (see Fernando et al., 1993), which are not currently supported by `PPP`. Attempting to run a `PPP` model for pedigrees with loops will result in an error. 

```{r, error = TRUE}
PPP(err_fam_1, cancers = c("Breast", "Pancreas"), genes = c("MSH2", "TP53"))
```
...
For more information about each of the sample families provided by the `PPP` package, please see their individual help pages. 
