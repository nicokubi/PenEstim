...
## MCMC with Independent MH 

The Independent MH algorithm is an MH algorithm whose proposal $P(\theta' \mid \theta_{t})$ does not depend on $\theta_{t}$, i.e., $P(\theta' \mid \theta_{t})=P(\theta')$.
...
save(out_sim_5.1, file = "out_sim_5.1.Rdata")

# print summary stats
out_sim_5.1$summary_stats
plot_traceSingle(out_sim_5.1$results[[1]])
plot_penetrance_sim(out_sim_5.1$combined_chains, prob = 0.95, max_age = 94,"Female",2.5,50,0,1)

# plot the scatterplot with the median and first quartile
plot(out_sim_5.1$combined_chains$median_male_results, out_sim_5.1$combined_chains$first_quartile_male_results, xlim = c(1,100), ylim = c(1,100))
abline(a=0, b=1, col="red") # This draws a red line with slope 1 passing through the origin
plot(out_sim_5.1$combined_chains$median_female_results, out_sim_5.1$combined_chains$first_quartile_female_results, xlim = c(1,100), ylim = c(1,100))
abline(a=0, b=1, col="red") # This draws a red line with slope 1 passing through the origin
```

## MCMC with Random Walk MH 

In this version of the MH proposals for new parameter values are generated using a normal distribution centered at the current parameter values. The standard deviation for these proposals is proportional to the current parameter values, scaled by a factor. 
The standard deviation for the proposal distribution is set as:
\[
\text{SD} = |\text{current parameter value}| \times \text{proportionality factor}
\]
where the proportionality factor is predefined (in this case, 0.2). This assumption implies that the variability of the proposals is directly linked to the magnitude of the current parameters, allowing for adaptive exploration depending on the parameter scale.

\section{Algorithm Workflow}
\begin{enumerate}
    \item Initialize parameters based on prior distributions and compute initial risks from demographic data.
    \item For each iteration:
    \begin{enumerate}
        \item Generate proposals for each parameter using the specified normal distribution.
        \item Calculate the likelihood and prior probability for both current and proposed parameters.
        \item Compute the acceptance ratio based on the likelihoods and priors.
        \item Decide acceptance of the proposal using a uniform random number compared to the acceptance ratio.
        \item Update parameters and store results.
    \end{enumerate}
    \item Return the chain of parameter samples along with acceptance and rejection statistics.
\end{enumerate}
...
# Run Estimation procedure with default prior setting 
# Main Estimation for Female
out_sim_5.2 <- PenEstim_v2(
    data = dat,
    cancer_type = "Breast", gene_input = "BRCA1", n_chains = 4, n_iter_per_chain = 2500, 
    prior_params = prior_params_BRCA1, af = 0.1, burn_in = 0.1, median_max = FALSE
)
save(out_sim_5.2, file = "out_sim_5.2.Rdata")

# print summary stats
out_sim_5.2$summary_stats
plot_traceSingle(out_sim_5.2$results[[1]])
plot_penetrance_sim(out_sim_5.2$combined_chains, prob = 0.95, max_age = 94,"Female",2.5,50,0,1)

# plot the scatterplot with the median and first quartile
plot(out_sim_5.2$combined_chains$median_male_results, out_sim_5.2$combined_chains$first_quartile_male_results, xlim = c(1,100), ylim = c(1,100))
abline(a=0, b=1, col="red") # This draws a red line with slope 1 passing through the origin
plot(out_sim_5.2$combined_chains$median_female_results, out_sim_5.2$combined_chains$first_quartile_female_results, xlim = c(1,100), ylim = c(1,100))
abline(a=0, b=1, col="red") # This draws a red line with slope 1 passing through the origin
```

## MCMC with Adaptive MH
