...
create_distributions <- function(
  ...
  if (is.null(dataframe) && is.null(samples_table) && !is.null(custom_params)) {
    ...
    if (!is.null(ratio)) {
      ...
      proposal_params$asymptote$g1 <- SERR_baseline * ratio
      proposal_params$asymptote$g2 <- SERR_baseline * ratio
    } else {
      proposal_params$asymptote <- compute_parameters_asymptote(max_age, risk_max_age)
    }
